...
16. `.gitignore` present at the project root and correct (`@rules/config.md §.gitignore`): ignores `node_modules/`, the anchored build outputs (`/out/`, `/dist/`), `*.vsix`, `tasks/`, `.claude/settings.local.json` + `.claude/agent-memory/`, and `docs/specs/`; **never** ignores `docs/release/CHANGELOG.md`, the root `CHANGELOG.md` mirror, `.claude/settings.json`, the generated `CLAUDE.md`, `.vscode/` (launch.json + tasks.json — F5 debug), `test/`/`tests/`, or `scripts/`. Verify with `git add -A` + `git status`, not only `git check-ignore` (the negation rules mislead it). Separate from `.vscodeignore` (`@rules/manifest.md`), which governs the `.vsix`.
