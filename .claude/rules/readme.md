...
A **version bump** does not trigger a regeneration either: `/vscode-release` itself updates the README **title line** (`# [APP_NAME] — v[VERSION]`) as a targeted edit of that single line (its Step 4 — Apply the release), so the Marketplace page never shows a stale version next to an up-to-date changelog.
