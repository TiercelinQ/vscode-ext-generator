...
- After each delivered batch, if the session is running long or the context is heavily loaded, offer `/vscode-save-session` before starting the next batch — the automatic chaining above stays the default.
